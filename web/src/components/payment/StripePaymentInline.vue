<template>
  <div class="space-y-4">
    <div v-if="loading" class="flex items-center justify-center py-12">
      <div class="h-8 w-8 animate-spin rounded-full border-4 border-slate-500 border-t-transparent"></div>
    </div>
    ...
    <template v-else-if="success">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <div class="w-full rounded-xl bg-slate-50 p-4 dark:bg-slate-800">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-slate-500 dark:text-slate-400">{{ t('payment.orders.orderId') }}</span>
                <span class="font-medium text-slate-900 dark:text-white">#{{ orderId }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-slate-500 dark:text-slate-400">{{ t('payment.orders.amount') }}</span>
                <span class="font-medium text-slate-900 dark:text-white">¥{{ payAmount.toFixed(2) }}</span>
              </div>
            </div>
          </div>
