...
const statusMap: Record<OrderStatus, { key: string; class: string }> = {
  ...
  EXPIRED: { key: 'payment.status.expired', class: 'bg-slate-100 text-slate-800 dark:bg-slate-900/30 dark:text-slate-400' },
  CANCELLED: { key: 'payment.status.cancelled', class: 'bg-slate-100 text-slate-800 dark:bg-slate-900/30 dark:text-slate-400' },
...
const statusClass = computed(() => {
  const entry = statusMap[props.status]
  return entry?.class ?? 'bg-slate-100 text-slate-800 dark:bg-slate-900/30 dark:text-slate-400'
})
</script>
