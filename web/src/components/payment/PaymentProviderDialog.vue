    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="form.provider_key === 'easypay'" class="flex items-center gap-2">
          <span class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ t('admin.settings.payment.paymentMode') }}</span>
            ...
            <button
              ...
              :class="[
                ...
                form.payment_mode === mode.value
                  ...
                  : 'border-slate-300 bg-white text-slate-600 hover:border-slate-400 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-slate-500',
              ]"
            >{{ mode.label }}</button>
          </div>
        </div>
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div class="border-t border-slate-200 pt-4 dark:border-slate-700">
        ...
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label }}
              <span v-if="field.optional" class="text-xs text-slate-400">({{ t('common.optional') }})</span>
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-slate-300 bg-slate-50 px-3 text-xs text-slate-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-400">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-slate-300 bg-slate-50 px-3 text-xs text-slate-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-400">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>

        <!-- Stripe webhook hint -->
        <div v-if="stripeWebhookUrl" class="mt-3 rounded-xl border border-blue-200 bg-blue-50 p-3 dark:border-blue-800/50 dark:bg-blue-900/20">
      ...
      <div v-if="limitableTypes.length" class="border-t border-slate-200 pt-4 dark:border-slate-700">
        <button type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          ...
          <svg :class="['h-4 w-4 text-slate-400 transition-transform', limitsExpanded && 'rotate-180']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          <div
            v-for="lt in limitableTypes"
            :key="lt.value"
            class="rounded-xl border border-slate-100 p-3 dark:border-slate-700"
          >
            <p class="mb-2 text-xs font-medium text-slate-700 dark:text-slate-300">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-slate-500 dark:text-slate-400">{{ t('admin.settings.payment.limitSingleMin') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-slate-500 dark:text-slate-400">{{ t('admin.settings.payment.limitSingleMax') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-slate-500 dark:text-slate-400">{{ t('admin.settings.payment.limitDaily') }}</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-slate-400 dark:text-slate-500">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>
