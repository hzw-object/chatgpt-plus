<template>
  <div class="md:hidden space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
        <div class="space-y-3">
          ...
          <div v-if="hasActionsColumn" class="border-t border-slate-200 pt-3 dark:border-slate-700">
            <div class="h-8 w-full animate-pulse rounded bg-slate-200 dark:bg-slate-700"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-xl border border-slate-200 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-900">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-slate-400 dark:text-slate-500"
            />
  ...
  >
    <table class="w-full min-w-max divide-y divide-slate-200 dark:divide-slate-700">
      <thead class="table-header bg-slate-50 dark:bg-slate-900">
        <tr>
          <th
            ...
            scope="col"
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400',
          ...
          >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span v-if="column.sortable" class="text-slate-400 dark:text-slate-500">
                  <svg
      ...
      <tbody class="table-body divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-900">
        <!-- Loading skeleton -->
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-slate-500 dark:text-slate-400', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          >
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-slate-900 dark:text-slate-100',
                getAdaptivePaddingClass(),
...
.table-wrapper .table-header {
  ...
  top: 0;
  z-index: 200;
  background-color: rgb(250 250 249); /* slate-50 */
}

.dark .table-wrapper .table-header {
  background-color: rgb(28 25 23); /* slate-900 */
}
...
.dark tbody tr:hover .sticky-col {
  background-color: rgb(41 37 36); /* slate-800 */
}
