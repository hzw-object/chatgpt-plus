<template>
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    ...
    <transition name="dropdown">
      <div
        v-if="tooltipOpen"
        class="absolute right-0 z-50 mt-2 w-[340px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800"
      >
        ...
        <div class="max-h-64 overflow-y-auto">
          <div
            ...
            :key="subscription.id"
            class="border-b border-slate-50 p-3 last:border-b-0 dark:border-slate-700/50"
          >
            ...
            <div class="space-y-1.5">
              ...
              <div
                ...
                class="flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-50 to-emerald-50 px-2.5 py-1.5 dark:from-emerald-900/20 dark:to-emerald-900/20"
              >
              ...
              <template v-else>
                <div v-if="subscription.group?.daily_limit_usd" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-slate-500">{{
                    t('subscriptionProgress.daily')
                  }}</span>
                  <div class="h-1.5 min-w-0 flex-1 rounded-full bg-slate-200 dark:bg-slate-600">
                    <div
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-slate-500">
                    {{
        ...
        <div class="border-t border-slate-100 p-2 dark:border-slate-700">
          <router-link
            to="/subscriptions"
            @click="closeTooltip"
            class="block w-full py-1 text-center text-xs text-violet-600 hover:underline dark:text-violet-400"
          >
...
function getProgressDotClass(sub: UserSubscription): string {
  ...
  if (maxPercentage >= 70) return 'bg-violet-500'
  return 'bg-green-500'
}
...
function getProgressBarClass(used: number | undefined, limit: number | null | undefined): string {
  ...
  if (percentage >= 70) return 'bg-violet-500'
  return 'bg-green-500'
}
...
function getDaysRemainingClass(expiresAt: string): string {
  ...
  if (days <= 7) return 'text-violet-600 dark:text-violet-400'
